...
contract NFT_VendingMachine is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC721Burnable,
    Ownable
{
    ...
    uint256 public max_supply = 100; // maximo de nft que a é possivel criar
    ...
    mapping(uint256 => bool) public sold; // retorna se um nft ja foi vendido
    ...
    function mint(string memory _tokenURI, uint256 value)
        public
        onlyOwner
        returns (bool)
    {
        require(totalSupply() < max_supply, "O limite de tokens foi atingido.");
        require(value > 0, unicode"[ERRO] Valor inválido"); // require para o value ser > 0
    ...
    function _validate(uint256 _id) internal {
        ...
        require(
            msg.value >= _value[_id],
            unicode"Error: Não há ether o suficiente!"
        );
    }

    function _trade(uint256 _id) internal {
        // primeiro envia o ether, depois envia o NFT = segurança
        payable(address(this)).transfer(msg.value); //eth para o contrato
        _transfer(address(this), msg.sender, _id); //nft para o usuário
        sold[_id] = true; //nft vendido
    }

    // função para mudar o valor do nft
    function changeTheValueNFT(uint256 _id, uint256 value) public onlyOwner {
    ...
    function sacar(uint256 _amount) public payable onlyOwner {
        require(
            ...
            unicode"ERRO: saldo indisponível no contrato!"
        );
        payable(owner()).transfer(_amount);
    }

    // essas funções são substituições exigidas pelo solidity.

    function _baseURI() internal pure override returns (string memory) {
        return
            "https://ipfs.io/ipfs/QmcoC6iGKjWdKcEjAzDz8apkCcz82ghTFHHNv3v397nRJA/";
    }
